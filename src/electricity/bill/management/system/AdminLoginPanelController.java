...
public class AdminLoginPanelController implements Initializable {
    ...
    public void login(ActionEvent event) throws Exception
    {
                    ...
                    if(in.Log("Admin",user_name,user_password))
                    {
                    loader.setLocation(getClass().getResource("AdminPanel.fxml"));
                    Parent adpanel = loader.load();
                    AdminPanelController controller= loader.getController();
                    controller.setName(user_name); 
                    Stage window = (Stage)((Node)event.getSource()).getScene().getWindow();
                    Scene adminpanel = new Scene(adpanel);
                    window.setScene(adminpanel);
                    window.setTitle("Admin Panel");
                    window.show();
                    }
                    else
                    {
                      JOptionPane.showMessageDialog(null, "Login Failed. Please Check your Name and Password ");
                    }
                    }
